...
export default class Home extends Component {
  state = {
    // Since this example is using the redirect flow, not a popup,
    // We need to check if we arrived at this page after the flow.
    // The Authorize component class exposes a static property to aid
    // in the flow. If we don't have a value, we can set whether we
    // want to kick off the flow immediately or not, using true or false.
    // Here we use false, so the user can kick off the flow with a button.
  ...
  render() {
    ...
    /**
     * 1. Without the `popup` prop, passing authorize={true} to the Authorize
     * component will redirect the page to the Auth0 login flow. Auth0 will
     * redirect back to the Login page when done.
     *
